...
async function main(): Promise<void> {
  const baseUrl = process.env.ATEL_TRUST_BASE_URL ?? 'http://127.0.0.1:3100';
  const mock = new MockAnchorProvider();

  const delegator = new ATELOrchestrator({
    agentId: 'qs-network-delegator',
    anchors: [mock],
  });
  const executor = new ATELOrchestrator({
    agentId: 'qs-network-executor',
    anchors: [mock],
    trustSync: new HttpTrustSyncAdapter({ baseUrl }),
  });

  const delegation = delegator.delegateTask({
    executor: executor.identity,
    intent: { type: 'quickstart_network', goal: 'Validate network mode' },
    risk: 'low',
  ...
  const execution = await executor.executeTask({
    ...
    execute: async (gateway) => gateway.callTool({
      tool: 'http.get',
      input: { url: 'https://mock.local/qs-network' },
      risk_level: 'low',
...
main().catch((err) => {
  console.error('quickstart-network failed:', err);
  process.exit(1);
});
